...
const NATIVE_AGENTS = [
  ...
  { key: 'company-signal-monitor', name: 'Company Signal Monitor', purpose: 'Continuously find timely, sourced developments about tracked companies.', capabilities: ['monitoring', 'exa-search', 'company-research'], schedule: { cadence: 'every-6-hours', cron: '35 */6 * * *', frequency_multiplier: 1 }, outputs: ['company', 'claim'], displayInRoster: true },
  { key: 'company-profile-curator', name: 'Company Profile Curator', purpose: 'Clean, reconcile, and maintain durable company intelligence profiles.', capabilities: ['enrichment', 'data-quality', 'exa-search'], schedule: { cadence: 'daily', cron: '10 5 * * *', interval_hours: 24, batch_limit: 4 }, outputs: ['company', 'claim'], displayInRoster: true },
  { key: 'company-discovery', name: 'Company Discovery', purpose: 'Source new companies that match the user taste profile and current interests.', capabilities: ['sourcing', 'preferences', 'exa-search'], schedule: { cadence: 'every-3-hours', cron: '50 */3 * * *', interval_hours: 3, batch_limit: 6 }, outputs: ['company_candidate'], displayInRoster: true },
  { key: 'evidence-auditor', name: 'Evidence Auditor', purpose: 'Audit new signals for support, contradictions, duplication, and staleness.', capabilities: ['verification', 'deduplication', 'confidence'], schedule: { cadence: 'hourly', cron: '20 * * * *' }, dependencies: ['company-signal-monitor', 'event-discovery'], outputs: ['claim', 'recommendation'], displayInRoster: true },
  { key: 'opportunity-investigator', name: 'Opportunity Investigator', purpose: 'Escalate promising, incomplete opportunities to deeper evidence-based investigation.', capabilities: ['deep-research', 'dust', 'recommendation'], schedule: { cadence: 'event-triggered' }, dependencies: ['company-signal-monitor', 'evidence-auditor', 'daily-candidate-ranking'], outputs: ['company', 'person', 'recommendation'], displayInRoster: true },
  { key: 'relationship-pathfinder', name: 'Relationship Pathfinder', purpose: 'Map the strongest direct, warm, contextual, or cold path into a company.', capabilities: ['relationship-graph', 'contact-mapping', 'recommendation'], schedule: { cadence: 'daily', cron: '25 7 * * *' }, dependencies: ['investor-map', 'firm-discovery'], outputs: ['person', 'recommendation'], displayInRoster: true },
  { key: 'follow-up-strategist', name: 'Follow-up Strategist', purpose: 'Recommend the next move for due and overdue actions without sending outreach.', capabilities: ['planning', 'follow-up', 'human-approval'], schedule: { cadence: 'daily', cron: '40 7 * * *' }, dependencies: ['relationship-pathfinder'], outputs: ['recommendation'], displayInRoster: true },
  { key: 'tuner-question-curator', name: 'Tuner Question Curator', purpose: 'Choose preference questions that reduce the most ranking uncertainty.', capabilities: ['preferences', 'active-learning', 'ranking'], schedule: { cadence: 'on-demand' }, dependencies: ['company-signal-monitor', 'evidence-auditor'], outputs: ['recommendation'] },
  { key: 'outcome-learning', name: 'Outcome Learning Agent', purpose: 'Learn transparent patterns from accepted signals and completed actions.', capabilities: ['outcomes', 'analysis', 'adaptation-proposal'], schedule: { cadence: 'weekly', cron: '20 6 * * 1' }, outputs: ['recommendation'], displayInRoster: true },
  { key: 'agent-portfolio-manager', name: 'Agent Portfolio Manager', purpose: 'Analyze agent cost, freshness, duplicates, failures, and accepted yield.', capabilities: ['economics', 'planning', 'adaptation-proposal'], schedule: { cadence: 'weekly', cron: '40 6 * * 1' }, dependencies: ['outcome-learning'], outputs: ['recommendation'], displayInRoster: true },
